...
if __name__ == "__main__":
    ...
    Model = CNN.CNN(param_conv, param_fcc, model_type="regression", training_generator=generator_training,
                    ...
                    verbose=1, num_gpu=1, lr=lr, loss=lf.sivia_skilling_loss, save_summary=True,
                    path_summary=path_model, validation_freq=1, train=True, compile=True)

    #### PREDICTIONS ###

    epoch = "50"
    model = load_model(path_model + "model/weights." + epoch + ".hdf5",
                       custom_objects={'sivia_skilling_loss':lf.sivia_skilling_loss})
