...
if __name__ == "__main__":
    ...
    path = "/home/luisals/test2/mse_log_alpha_-6/run2/"
    path_sims = "/lfstev/deepskies/luisals/"
    # path = "/mnt/beegfs/work/ati/pearl037/regression/test/mse/"
    # path_sims = "/mnt/beegfs/work/ati/pearl037/"

    all_sims = ["0", "1", "2", "4", "5", "6"]
    s = tn.SimulationPreparation(all_sims, path=path_sims)

    training_particle_IDs = load(open('/home/luisals/test2/training_set.pkl', 'rb'))
    training_labels_particle_IDS = load(open('/home/luisals/test2/labels_training_set.pkl', 'rb'))

    val_particle_IDs = load(open('/home/luisals/test2/validation_set.pkl', 'rb'))
    val_labels_particle_IDS = load(open('/home/luisals/test2/labels_validation_set.pkl', 'rb'))
    ...
    Model = CNN.CNN(param_conv, param_fcc, model_type="regression", training_generator=generator_training,
                    validation_generator=generator_validation, callbacks=[csv_logger], num_epochs=10,
                    dim=generator_training.dim, max_queue_size=10, use_multiprocessing=False, workers=0, verbose=1,
                    num_gpu=1, lr=0.0001, save_summary=True, path_summary=path, validation_freq=1, train=True,
                    compile=True)

np.savetxt(path + 'loss.txt', Model.model.history.history['loss'])
np.savetxt(path + 'val_loss.txt', Model.model.history.history['val_loss'])
