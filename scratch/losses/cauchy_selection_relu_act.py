...
if __name__ == "__main__":
    ...
    if training:
        ...
        Model = CNN.CNN(param_conv, param_fcc, model_type="regression", training_generator=generator_training,
                        validation_generator=generator_validation, metrics=['mae', 'mse'], num_epochs=10,
                        dim=generator_training.dim, max_queue_size=10, use_multiprocessing=False, workers=0, verbose=1,
                        num_gpu=1, lr=0.0001, loss=lf.cauchy_selection_loss(), save_summary=True,
                        path_summary=path_model, validation_freq=1, train=False, compile=True,
                        validation_steps=len(generator_validation), steps_per_epoch=1000)
    ...
    if testing:
        lr = 0.0001
        Model = CNN.CNN(param_conv, param_fcc, model_type="regression", training_generator=generator_training,
                        metrics=['mae', 'mse'], num_epochs=11, dim=generator_training.dim, max_queue_size=10,
                        use_multiprocessing=False, workers=0, verbose=1, num_gpu=1, lr=0.0001,
                        loss=lf.cauchy_selection_loss(), save_summary=True, path_summary=path_model, validation_freq=1,
                        train=False, compile=False, initial_epoch=10)
