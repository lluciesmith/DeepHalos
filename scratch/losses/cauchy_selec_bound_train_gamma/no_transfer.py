...
if __name__ == "__main__":
    ...
    path_model = "/lfstev/deepskies/luisals/regression/large_CNN/test_lowmass/reg_10000_perbin/larger_net/lr_decay" \
                 "/cauchy_selec_bound_gamma_no_transfer/"
    ...
    lr = 0.0001
    Model = CNN.CNN(param_conv, param_fcc, model_type="regression", training_generator=generator_training, num_epochs=1,
                    dim=generator_validation.dim, max_queue_size=10, use_multiprocessing=False, workers=0, verbose=1,
                    num_gpu=1, lr=lr, loss='mse', save_summary=True, path_summary=path_model, validation_freq=1,
                    train=False)

    # Define new model

    m = Model.model
    predictions = CNN.LossTrainableParams(init_gamma=0.2)(m.layers[-1].output)
