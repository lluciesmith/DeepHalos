...
if __name__ == "__main__":
    ...
    Model = CNN.CNNCauchy(param_conv, param_fcc, model_type="regression", training_generator={}, shuffle=True,
                          validation_generator={}, num_epochs=100, dim=(75, 75, 75), initialiser=initialiser,
                          max_queue_size=80, use_multiprocessing=True, workers=40, verbose=1, num_gpu=1, lr=0.0001,
                          save_summary=False, path_summary=saving_path, validation_freq=1, train=False, compile=True,
                          initial_epoch=None, seed=seed)
    ...
    activations = []
    for i in [1, 2, 4, 6, 8, 10, 13, 14, 15]:
        f = layers[i].output
        output_layers = sess.run(f, feed_dict={Model.model.input: np.float32(g[0])})
        np.save(saving_path + "output_layer_" + str(i) + "_Xavier_relu.npy", output_layers)
...
#
#         f, axes = plt.subplots(2, len(act), figsize=(13, 8), sharex=True)
#
