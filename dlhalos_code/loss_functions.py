...
def conditional_loss(y_true, y_predicted):
    ...
    mask_neg = K.less(y_predicted, np.zeros_like(y_predicted)) # this means that is is 1 when predictions < -1
    #
    ...
    loss = tf.where(mask_neg, K.square(y_true - y_predicted), K.abs(y_true - y_predicted))
