    ...
    def compile_cauchy_model(self, mse_model):
        ...
        if self.init_alpha is not None:
            for layer in new_model.layers[:-2]:
                if 'conv3d' in layer.name:
                    print(layer)
                    new_model.add_loss(lambda: last_layer.alpha/2 * custom_reg.l2_norm(1.)(layer.kernel))
                elif 'dense' in layer.name:
                    print(layer)
                    new_model.add_loss(lambda: last_layer.alpha/2 * custom_reg.l1_and_l21_group(1.)(layer.kernel))
                else:
                    pass
