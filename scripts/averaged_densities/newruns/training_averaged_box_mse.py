...
if __name__ == "__main__":
    ...
    pred = Model.model.predict(tset, verbose=1)
    truth_rescaled = np.array([params.training_labels_particle_IDS[ID] for ID in params.training_particle_IDs])
    h_m_pred = params.scaler.inverse_transform(pred.reshape(-1, 1)).flatten()
    true = params.scaler.inverse_transform(truth_rescaled.reshape(-1, 1)).flatten()
    np.save(params.saving_path + "predicted_sim_testset_epoch_%02d.npy" % num_epoch_testing, h_m_pred)
    np.save(params.saving_path + "true_sim_testset_epoch_%02d.npy" % num_epoch_testing, true)

    y = np.concatenate([y for x, y in tset], axis=0)
    true = params.scaler.inverse_transform(y.reshape(-1, 1)).flatten()
    np.save(params.saving_path + "true_sim_testset_epoch_%02d.npy" % num_epoch_testing, true)


    w = h5py.File(params.saving_path + "model/weights.%02d.h5" % num_epoch_testing, 'r')
    w[0]
