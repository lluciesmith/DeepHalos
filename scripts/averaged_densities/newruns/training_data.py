...
if __name__ == "__main__":
    ...
    training_set = tn.InputsPreparation(train_sims, shuffle=True, scaler_type="minmax", return_rescaled_outputs=True,
                                        output_range=(-0.95, 0.95), log_high_mass_limit=13.4, log_low_mass_limit=11,
                                        load_ids=False, random_subset_each_sim=None,
                                        random_style="random", random_subset_all=n_samples,
                                        path=path_sims)
    dump(training_set.particle_IDs, open(saving_path + 'training_set_400k.pkl', 'wb'))
    dump(training_set.labels_particle_IDS, open(saving_path + 'labels_training_set_400k.pkl', 'wb'))
    dump(training_set.scaler_output, open(saving_path + 'scaler_output_400k.pkl', 'wb'))
    ...
    dump(v_set.particle_IDs, open(saving_path + 'validation_set_400k.pkl', 'wb'))
    dump(v_set.labels_particle_IDS, open(saving_path + 'labels_validation_set_400k.pkl', 'wb'))
    ...
    dump(test_setIDs, open(saving_path + 'test_set_400k.pkl', 'wb'))
    dump(labels_test_setIDs, open(saving_path + 'labels_test_set_400k.pkl', 'wb'))
